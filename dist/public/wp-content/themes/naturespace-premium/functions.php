 ...
 *  
*/    
$naturespace_themename = "NatureSpace Premium";			//Theme Name
$naturespace_themever = "2.0.1";									//Theme version
...
require_once ($naturespace_be_path . 'fw-setup.php');		   // Init 
...
require_once ($naturespace_fe_path . 'widget-twitter.php'); // Twitter Widget
require_once ($naturespace_fe_path . 'widget-facebook.php');// Facebook Widget
require_once ($naturespace_fe_path . 'widget-info-box.php');// Box-Info Widget
require_once ($naturespace_fe_path . 'widget-image-box.php');// Box-Image Widget
require_once ($naturespace_fe_path . 'widget-presentation-box.php');// Box-Presentation Widget
require_once ($naturespace_fe_path . 'widget-section-headline.php');// Section-Headline Widget
require_once ($naturespace_fe_path . 'widget-posts-column.php');// Posts-Column Widget
require_once ($naturespace_fe_path . 'widget-posts-default.php');// Posts-Default Widget
require_once ($naturespace_fe_path . 'widget-posts-grid.php');// Posts-Grid Widget
require_once ($naturespace_fe_path . 'widget-posts-list.php');// Posts-List Widget
require_once ($naturespace_fe_path . 'widget-posts-slider.php');// Posts-Slider Widget
require_once ($naturespace_fe_path . 'widget-posts-thumbnail.php');// Posts-Thumbnail Widget
require_once ($naturespace_fe_path . 'google-map-shortcode.php');// Google Maps Shortcode
require_once ($naturespace_fe_path . '/wp-display-header/wp-display-header.php');// Different Header Images for individual pages
require_once ($naturespace_fe_path . '/wp-tab-widget/wp-tab-widget.php');// Tab Widget
...
function naturespace_scripts_styles() {
	global $wp_styles, $naturespace_options; 
  foreach ($naturespace_options as $value) {
    ...
    if ( $naturespace_display_carousel != 'Hide' && !is_page_template('template-landing-page.php') ) {  
    wp_enqueue_script( 'naturespace-caroufredsel', get_template_directory_uri() . '/js/caroufredsel.js', array( 'jquery' ), '6.2.1', true );
    wp_enqueue_script( 'naturespace-caroufredsel-settings', get_template_directory_uri() . '/js/caroufredsel-settings.js', array(), '1.0', true ); } 
    if ( $naturespace_post_entry_format == 'Grid - Masonry' ) {
    if ( is_home() || is_archive() || is_search() ) { 
    wp_enqueue_script( 'naturespace-imagesloaded', get_template_directory_uri() . '/js/imagesloaded.js', array(), '3.1.8', true );
    wp_enqueue_script( 'naturespace-masonry', get_template_directory_uri() . '/js/masonry.js', array(), '3.1.5', true );
    if ( !is_rtl() ) {
    ...
    wp_enqueue_script( 'naturespace-flexslider', get_template_directory_uri() . '/js/flexslider.js', array( 'jquery' ), '2.2.0', true );
    if ( $naturespace_layout_width == 'Wide' ) {
    wp_enqueue_script( 'naturespace-flexslider-settings', get_template_directory_uri() . '/js/flexslider-settings.js', array( 'jquery' ), '2.2.0', true ); } else {
    wp_enqueue_script( 'naturespace-flexslider-settings-boxed', get_template_directory_uri() . '/js/flexslider-settings-boxed.js', array( 'jquery' ), '2.2.0', true ); }
    ...
    if ( has_nav_menu( 'main-navigation' ) && $naturespace_header_layout == 'Centered' && !is_page_template('template-landing-page.php') ) {
    ...
    wp_enqueue_style( 'naturespace-tab-widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/css/wp-tab-widget.css' );
 ...
 *  
*/
function naturespace_catch_that_image() {
  global $post, $posts;
  $first_img = '';
  ob_start();
  ob_end_clean();
 ...
 *  
*/
function naturespace_catch_that_audio() {
global $post, $posts;
...
/**
 * Adds a custom meta box for setting a header slideshow.
 *
*/
function naturespace_add_meta_box_slideshow() {
	$screens = array( 'post', 'page' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'naturespace_sectionid_slideshow',
			__( 'Header Slideshow', 'naturespace' ),
			'naturespace_meta_box_callback_slideshow',
			$screen
		);
	}
}
add_action( 'add_meta_boxes', 'naturespace_add_meta_box_slideshow' );

function naturespace_meta_box_callback_slideshow( $post ) {
	wp_nonce_field( 'naturespace_meta_box_slideshow', 'naturespace_meta_box_nonce_slideshow' );
	$value = get_post_meta( $post->ID, '_my_meta_value_key_slideshow', true );
	echo '<label for="naturespace_new_field_slideshow">';
	_e( 'Insert here a Slideshow ID to display a slideshow in header.', 'naturespace' );
	echo '</label> ';
	echo '<input type="text" id="naturespace_new_field_slideshow" name="naturespace_new_field_slideshow" value="' . esc_attr( $value ) . '" size="100" />';
}

function naturespace_save_meta_box_data_slideshow( $post_id ) {
	if ( ! isset( $_POST['naturespace_meta_box_nonce_slideshow'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['naturespace_meta_box_nonce_slideshow'], 'naturespace_meta_box_slideshow' ) ) {
		return;
	}
	...
	if ( ! isset( $_POST['naturespace_new_field_slideshow'] ) ) {
		return;
	}
	$my_data = sanitize_text_field( $_POST['naturespace_new_field_slideshow'] );
	update_post_meta( $post_id, '_my_meta_value_key_slideshow', $my_data );
}
add_action( 'save_post', 'naturespace_save_meta_box_data_slideshow' );

/**
 * Adds a custom meta box for setting a Video Thumbnail.
 *
*/
function naturespace_add_meta_box() {
	$screens = array( 'post' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'naturespace_sectionid',
			__( 'Video Thumbnail', 'naturespace' ),
			'naturespace_meta_box_callback',
			$screen
		);
	}
}
add_action( 'add_meta_boxes', 'naturespace_add_meta_box' );

function naturespace_meta_box_callback( $post ) {
	wp_nonce_field( 'naturespace_meta_box', 'naturespace_meta_box_nonce' );
	$value = get_post_meta( $post->ID, '_my_meta_value_key', true );
	echo '<label for="naturespace_new_field">';
	_e( 'Insert here the full URL address of the Video Thumbnail.', 'naturespace' );
	echo '</label> ';
	echo '<input type="text" id="naturespace_new_field" name="naturespace_new_field" value="' . esc_attr( $value ) . '" size="25" />';
}

function naturespace_save_meta_box_data( $post_id ) {
	if ( ! isset( $_POST['naturespace_meta_box_nonce'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['naturespace_meta_box_nonce'], 'naturespace_meta_box' ) ) {
		return;
	}
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	...
	if ( ! isset( $_POST['naturespace_new_field'] ) ) {
		return;
	}
	$my_data = sanitize_text_field( $_POST['naturespace_new_field'] );
	update_post_meta( $post_id, '_my_meta_value_key', $my_data );
}
add_action( 'save_post', 'naturespace_save_meta_box_data' );
 ...
 *
 */
function naturespace_register_my_menus() {
  register_nav_menus(
    array(
      'main-navigation' => __( 'Main Header Menu', 'naturespace' ),
      'top-navigation' => __( 'Social Networks Links', 'naturespace' ),
      'carousel-navigation' => __( 'Carousel Menu', 'naturespace' )
    )
  );
}
add_action( 'after_setup_theme', 'naturespace_register_my_menus' );
...
function naturespace_widgets_init() {
  ...
  register_sidebar( array(
		'name' => __( 'Homepage slideshow', 'naturespace' ),
		'id' => 'sidebar-6',
		'description' => __( 'The area for Cyclone Slider Widget which displays a slideshow on your homepage.', 'naturespace' ),
 ...
 *
*/
function naturespace_short_title() {
$title = get_the_title();
$limit = "18";
$pad = "...";
 ...
 *
*/
function naturespace_status_content() {
 ...
 *  
*/
function naturespace_panels_row_styles($styles) {
    $styles['naturespace'] = __('NatureSpace', 'naturespace');
    $styles['naturespace-fullscreen'] = __('NatureSpace - full screen', 'naturespace');
    return $styles;
}
add_filter('siteorigin_panels_row_styles', 'naturespace_panels_row_styles');

/**
 * Custom Shortcodes.
 *
*/
function naturespace_button_shortcode($atts, $content = null) {
   extract(shortcode_atts(array('link' => '#'), $atts));
   return '<a class="custom-button" href="'.$link.'">' . do_shortcode($content) . '</a>';
}
add_shortcode( 'button', 'naturespace_button_shortcode' );
add_filter('widget_text', 'do_shortcode');

function naturespace_highlight_shortcode($atts, $content = null) {
   return '<span class="highlight-text">' . do_shortcode($content) . '</span>';
}
add_shortcode( 'highlight', 'naturespace_highlight_shortcode' );

function naturespace_table_shortcode($atts) {
...
add_shortcode( 'table', 'naturespace_table_shortcode' );

function naturespace_image_shortcode($atts, $content = null){
return '<img src="'.$atts['src'].'" alt="" />';
}
add_shortcode('image','naturespace_image_shortcode');

/**
 * Shortcode for displaying lists of posts.
 *  
*/
add_shortcode( 'display-posts', 'naturespace_display_posts_shortcode' );
function naturespace_display_posts_shortcode( $atts ) {
...
$example_update_checker = new ThemeUpdateChecker(
    'naturespace-premium',
    'http://www.tomastoman.cz/updates/naturespace.json'
); ?>