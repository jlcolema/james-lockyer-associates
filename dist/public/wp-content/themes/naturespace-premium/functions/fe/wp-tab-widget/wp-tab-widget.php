<?php
/*
Plugin Name: NatureSpace Tab Widget
...
class naturespace_widget extends WP_Widget {
	function __construct() {
        
		// ajax functions
		add_action('wp_ajax_naturespace_widget_content', array(&$this, 'ajax_naturespace_widget_content'));
		add_action('wp_ajax_nopriv_naturespace_widget_content', array(&$this, 'ajax_naturespace_widget_content'));
		...
		$widget_ops = array('classname' => 'widget_wpt', 'description' => __('Display popular posts, recent posts, comments and tags in tabbed format.', 'naturespace'));
		$control_ops = array('width' => 300, 'height' => 350);
		parent::__construct('naturespace_widget', __('NatureSpace Tab Widget', 'naturespace'), $widget_ops, $control_ops);
    }	

    function wpt_admin_scripts($hook) {
        wp_enqueue_script( 'naturespace_widget_admin', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wpt-admin.js', array( 'jquery' ), '1.0', true );
    }
    function wpt_register_scripts() { 
		// JS    
		wp_enqueue_script('naturespace_widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wp-tab-widget.js', array('jquery'), '1.0', true);     
		wp_localize_script( 'naturespace_widget', 'wpt',         
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'naturespace'); ?></h4>
		...
		<div class="wpt_select_tabs">
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 5px" for="<?php echo $this->get_field_id("tabs"); ?>_popular">
				...
				<?php _e( 'Popular Tab', 'naturespace'); ?>
			</label>
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_comments">
				...
				<?php _e( 'Comments Tab', 'naturespace'); ?>
			</label>
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_tags">
				...
				<?php _e( 'Tags Tab', 'naturespace'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <h4 class="wpt_tab_order_header"><a href="#"><?php _e('Tab Order', 'naturespace'); ?></a></h4>
        ...
        <div class="wpt_tab_order" style="display: none;">
            ...
            <label class="alignleft" for="<?php echo $this->get_field_id('tab_order'); ?>_tags" style="width: 50%;">
			    ...
			    <?php _e('Tags', 'naturespace'); ?>
            </label>
        </div>
		<div class="clear"></div>
        
        <h4 class="wpt_advanced_options_header"><a href="#"><?php _e('Advanced Options', 'naturespace'); ?></a></h4>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'naturespace'); ?>
			</label>
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of posts to show:', 'naturespace'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb"); ?>">
				...
				<?php _e( 'Show post thumbnails', 'naturespace'); ?>
			</label>
		</p>   	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show post date', 'naturespace'); ?>	
			</label>	
		</p>
		...
		<p>		
			<label for="<?php echo $this->get_field_id("show_comment_num"); ?>">		
				...
				<?php _e( 'Show number of comments', 'naturespace'); ?>		
			</label>	
		</p>    
        ...
        <p>
			<label for="<?php echo $this->get_field_id('comment_num'); ?>">
				<?php _e('Number of comments on Comments Tab:', 'naturespace'); ?>
				<br />
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_avatar"); ?>">			
				...
				<?php _e( 'Show avatars on Comments Tab', 'naturespace'); ?>	
			</label>	
		</p>
	...
	function widget( $args, $instance ) {	
		extract($args);     
		extract($instance);    
		wp_enqueue_script('naturespace_widget');  
        ...
        $available_tabs = array('popular' => __('Popular', 'naturespace'), 
            'recent' => __('Recent', 'naturespace'), 
            'comments' => __('Comments', 'naturespace'), 
            'tags' => __('Tags', 'naturespace'));
            
        array_multisort($tab_order, $available_tabs);
        
		?>	
		<?php echo $before_widget; ?>	
		<div class="naturespace_widget_content" id="<?php echo $widget_id; ?>_content">		
	...
	function ajax_naturespace_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
		...
		switch ($tab) {        
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>	
								<div class="wpt-postmeta">						
									...
									<?php if ( $show_comment_num == 1 ) : ?>			
										<?php echo comments_number(__('No Comment','naturespace'), __('One Comment','naturespace'), '<span class="comments-number">%</span> '.__('Comments','naturespace'));?>				
									<?php endif; ?>						
								</div> <!--end .entry-meta--> 				
							<?php endif; ?>
						...
						<li>
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>			
								<div class="wpt-postmeta">										
									...
									<?php endif; ?>								
									<?php if ( $show_comment_num == 1 ) : ?>	
										<?php echo comments_number(__('No Comment','naturespace'), __('One Comment','naturespace'), '<span class="comm">%</span> '.__('Comments','naturespace'));?>									
									<?php endif; ?>		
								</div> <!--end .entry-meta--> 		
							<?php endif; ?>
			...
			case "comments":         
				...
				<ul>            
					...
					<?php endforeach; else : ?>           
						<li>                   
							<div class="no-comments"><?php _e('No comments yet.', 'naturespace'); ?></div>        
						</li>                             
			...
			case "tags":        
				...
				<ul>         
					...
					if($tags) {               
						...
						_e('No tags created.', 'naturespace');           
					}            
					?>           
				</ul>            
				<?php            
    ...
    function tab_pagination($page, $last_page) {  
		...
		<div class="wpt-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&larr; Previous', 'naturespace'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &rarr;', 'naturespace'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "naturespace_widget" );' ) );

// unregister MTS Tabs Widget and Tabs Widget v2
add_action('widgets_init', 'naturespace_unregister_mts_tabs_widget', 100);
function naturespace_unregister_mts_tabs_widget() {
