<?php
/**
 * Plugin Name: NatureSpace Twitter Widget
...
?>
<?php 
add_action('widgets_init', create_function('', 'return register_widget("naturespacetwitter");'));
class naturespacetwitter extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'naturespacetwitter', 'description' => __('Twitter widget by NatureSpace', 'naturespace') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('naturespacettwi', __('NatureSpace Twitter Widget', 'naturespace'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		if ( $instance ) {
			...
			$title = __( 'Join us on Twitter', 'naturespace' );
		} 
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>">
				<?php _e('Twitter Username:', 'naturespace'); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" />
		<p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
			<?php _e('Insert your Twitter username. (ex: naturespace).', 'naturespace'); ?>
		</p>
		</p>
		<?php 
	} 
	...
	function widget($args, $instance) {
		...
		function naturespacemakeURL($url) {
		...
		foreach ($feed->get_items(0,$tweets) as $item) { ?>
			<p class="tweet-text"><?php echo naturespaceuserurl(naturespacetagurl(naturespacemakeURL($item->get_description()))); ?> 
