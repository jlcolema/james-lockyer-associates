<?php
/**
 * Plugin Name: MeadowHill Social Widget
 * Description: Display links to profiles on the most popular social networks.
 * Author: WPlook Team and Tomas Toman	
 * Version: 1.3
*/

add_action('widgets_init', create_function('', 'return register_widget("meadowhillsocial");'));
class meadowhillsocial extends WP_Widget {
	function meadowhillsocial() {
		$widget_ops = array('classname' => 'meadowhillsocial', 'description' => __('A Social widget by MeadowHill', 'meadowhill') );
		$control_ops = array('width' => 200, 'height' => 400);
		$this->WP_Widget('meadowhilltsoc', __('MeadowHill Social Widget', 'meadowhill'), $widget_ops, $control_ops);
	}
...
if ( $instance ) {
	...
	if ( $instance ) {
        ...
        if ( $instance ) {
			...
			$googleplus = __( '', 'meadowhill' );
		} 
        if ( $instance ) {
			$youtube = esc_attr( $instance[ 'youtube' ] );
		}
		else {
			$youtube = __( '', 'meadowhill' );
		}
		...
		if ( $instance ) {
			...
			$linkedin = __( '', 'meadowhill' );
		} 
        if ( $instance ) {
			$delicious = esc_attr( $instance[ 'delicious' ] );
		}
		else {
			$delicious = __( '', 'meadowhill' );
		} 
        if ( $instance ) {
			$pinterest = esc_attr( $instance[ 'pinterest' ] );
		}
		else {
			$pinterest = __( '', 'meadowhill' );
		}
     	 if ( $instance ) {
			$flickr = esc_attr( $instance[ 'flickr' ] );
		}
		else {
			$flickr = __( '', 'meadowhill' );
		}		
		?>
<!-- Title-->
<p>
	<label for="<?php echo $this->get_field_id('title'); ?>">
		<?php _e('Title:', 'meadowhill'); ?>
	</label>
...
<p>
	<label for="<?php echo $this->get_field_id('facebook'); ?>"> <img style="float: left; margin: 0 5px 0px 0; width: 12px; height: 12px;" src="<?php echo get_template_directory_uri()  ?>/functions/be/images/icons/facebook.png" />
		<?php _e('Facebook:', 'meadowhill'); ?>
	</label>
	<input class="widefat" id="<?php echo $this->get_field_id('facebook'); ?>" name="<?php echo $this->get_field_name('facebook'); ?>" type="text" value="<?php echo $facebook; ?>" />
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your Facebook profile, page or group.', 'meadowhill'); ?>
</p>
</p>
<!-- RSS-->
<p>
...
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your Google Plus profile', 'meadowhill'); ?>
</p>
</p>
<!-- You Tube-->
<p>
...
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your YouTube profile.', 'meadowhill'); ?>
</p>
</p>
<!-- Linkedin-->
<p>
...
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your Linkedin profile.', 'meadowhill'); ?>
</p>
</p>
<!-- Delicious-->
<p>
...
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your Delicious profile.', 'meadowhill'); ?>
</p>
</p>
<!--Pinterest-->
<p>
...
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your Pinterest profile.', 'meadowhill'); ?>
</p>
</p>
<!-- Flickr-->
<p>
...
<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
	<?php _e('Insert the full URL of your Flickr profile.', 'meadowhill'); ?>
</p>
</p>
<?php 
