<?php
/**
 * Plugin Name: ForeverWood Posts-Slider Widget
 * Description: Displays the latest posts from the selected category as a slider.
 * Author: Tomas Toman	
 * Version: 1.0
*/

add_action('widgets_init', create_function('', 'return register_widget("foreverwood_homepage_slider");'));
class foreverwood_homepage_slider extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'homepage-slider-posts', 'description' => __('Displays the latest posts from the selected category as a slider.', 'foreverwood') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('foreverwoodslider', __('ForeverWood Posts-Slider', 'foreverwood'), $widget_ops, $control_ops);
	}
...
<?php if ($my_query->have_posts()) : while ($my_query->have_posts()) : $my_query->the_post(); ?>            
          ...
          <a title="<?php the_title(); ?>" href="<?php echo get_permalink(); ?>"><?php the_post_thumbnail( 'slider-thumb' ); ?></a><?php endif; ?><a title="<?php the_title(); ?>" class="slider-link" href="<?php echo get_permalink(); ?>"><?php foreverwood_short_title(); ?></a></li>
<?php endwhile; endif; ?>
<?php wp_reset_query(); ?>
        </ul>
      </div>
