<?php
/**
 * Plugin Name: ForeverWood Twitter Widget
...
?>
<?php 
add_action('widgets_init', create_function('', 'return register_widget("foreverwoodtwitter");'));
class foreverwoodtwitter extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'foreverwoodtwitter', 'description' => __('Twitter widget by ForeverWood', 'foreverwood') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('foreverwoodttwi', __('ForeverWood Twitter Widget', 'foreverwood'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		if ( $instance ) {
			...
			$title = __( 'Join us on Twitter', 'foreverwood' );
		} 
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>">
				<?php _e('Twitter Username:', 'foreverwood'); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" />
		<p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
			<?php _e('Insert your Twitter username. (ex: foreverwood).', 'foreverwood'); ?>
		</p>
		</p>
		<?php 
	} 
	...
	function widget($args, $instance) {
		...
		function foreverwoodmakeURL($url) {
		...
		foreach ($feed->get_items(0,$tweets) as $item) { ?>
			<p class="tweet-text"><?php echo foreverwooduserurl(foreverwoodtagurl(foreverwoodmakeURL($item->get_description()))); ?> 
