<?php
/**
 * Plugin Name: DaisyChain Posts-List Widget
 * Description: Displays the latest posts from the selected category as an unordered list with highlighted first post.
 * Author: Tomas Toman	
 * Version: 1.0
*/

add_action('widgets_init', create_function('', 'return register_widget("daisychain_homepage_list");'));
class daisychain_homepage_list extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'homepage-list-posts', 'description' => __('Displays the latest posts from the selected category as an unordered list with highlighted first post.', 'daisychain') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('daisychainlist', __('DaisyChain Posts-List', 'daisychain'), $widget_ops, $control_ops);
	}
