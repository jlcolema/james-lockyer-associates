<?php
/**
 * Plugin Name: LiveRide Twitter Widget
...
?>
<?php 
add_action('widgets_init', create_function('', 'return register_widget("liveridetwitter");'));
class liveridetwitter extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'liveridetwitter', 'description' => __('Twitter widget by LiveRide', 'liveride') );
		$control_ops = array('width' => 200, 'height' => 400);
		parent::__construct('liveridettwi', __('LiveRide Twitter Widget', 'liveride'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		if ( $instance ) {
			...
			$title = __( 'Join us on Twitter', 'liveride' );
		} 
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>">
				<?php _e('Twitter Username:', 'liveride'); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" />
		<p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
			<?php _e('Insert your Twitter username. (ex: liveride).', 'liveride'); ?>
		</p>
		</p>
		<?php 
	} 
	...
	function widget($args, $instance) {
		...
		foreach ($feed->get_items(0,$tweets) as $item) { ?>
			<p class="tweet-text"><?php echo liverideuserurl(liveridetagurl(liveridemakeURL($item->get_description()))); ?> 
