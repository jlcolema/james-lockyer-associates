 ...
 *  
*/    
$liveride_themename = "LiveRide Premium";			//Theme Name
$liveride_themever = "1.0.9";									//Theme version
...
require_once ($liveride_be_path . 'fw-setup.php');		   // Init 
...
require_once ($liveride_fe_path . 'widget-social.php');	 // Social Widget
require_once ($liveride_fe_path . 'widget-twitter.php'); // Twitter Widget
require_once ($liveride_fe_path . 'widget-facebook.php');// Facebook Widget
require_once ($liveride_fe_path . 'widget-info-box.php');// Info-Box Widget
require_once ($liveride_fe_path . 'widget-posts-column.php');// Posts-Column Widget
require_once ($liveride_fe_path . 'widget-posts-default.php');// Posts-Default Widget
require_once ($liveride_fe_path . 'widget-posts-grid.php');// Posts-Grid Widget
require_once ($liveride_fe_path . 'widget-posts-list.php');// Posts-List Widget
require_once ($liveride_fe_path . 'widget-posts-slider.php');// Posts-Slider Widget
require_once ($liveride_fe_path . 'widget-posts-thumbnail.php');// Posts-Thumbnail Widget
require_once ($liveride_fe_path . 'google-map-shortcode.php');// Google Maps Shortcode
require_once ($liveride_fe_path . '/wp-display-header/wp-display-header.php');// Different Header Images for individual pages
require_once ($liveride_fe_path . '/wp-tab-widget/wp-tab-widget.php');// Tab Widget
...
function liveride_setup() {
  ...
  add_image_size( 'grid-thumb', 184, 138, true );
  add_image_size( 'square-thumb', 60, 60, true );
  add_image_size( 'slider-thumb', 140, 105, true );
  add_image_size( 'thumbnail-thumb', 196, 147, true );
...
function liveride_scripts_styles() {
    ...
    wp_enqueue_script( 'liveride-flexslider', get_template_directory_uri() . '/js/flexslider.js', array( 'jquery' ), '2.2.2', true );
    wp_enqueue_script( 'liveride-flexslider-settings', get_template_directory_uri() . '/js/flexslider-settings.js', array( 'jquery' ), '2.2.0', true );
    ...
    wp_enqueue_style( 'liveride-tab-widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/css/wp-tab-widget.css' );
...
add_action( 'liveride_register', 'liveride_my_theme_register_required_plugins' );
 ...
 *  
*/
function liveride_panels_row_styles($styles) {
    $styles['liveride'] = __('LiveRide', 'liveride');
    return $styles;
}
add_filter('siteorigin_panels_row_styles', 'liveride_panels_row_styles');

/**
 * Custom Shortcodes.
 *
*/
function liveride_button_shortcode($atts, $content = null) {
   extract(shortcode_atts(array('link' => '#'), $atts));
   return '<a class="custom-button" href="'.$link.'">' . do_shortcode($content) . '</a>';
}
add_shortcode( 'button', 'liveride_button_shortcode' );
add_filter('widget_text', 'do_shortcode');

function liveride_highlight_shortcode($atts, $content = null) {
   return '<span class="highlight-text">' . do_shortcode($content) . '</span>';
}
add_shortcode( 'highlight', 'liveride_highlight_shortcode' );

function liveride_table_shortcode($atts) {
...
add_shortcode( 'table', 'liveride_table_shortcode' );

function liveride_image_shortcode($atts, $content = null){
return '<img src="'.$atts['src'].'" alt="" />';
}
add_shortcode('image','liveride_image_shortcode');

/**
 * Shortcode for displaying lists of posts.
 *  
*/
add_shortcode( 'display-posts', 'liveride_display_posts_shortcode' );
function liveride_display_posts_shortcode( $atts ) {
...
$example_update_checker = new ThemeUpdateChecker(
    'liveride-premium',
    'http://www.tomastoman.cz/updates/liveride.json'
); ?>